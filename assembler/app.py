...
def build_audio_track(audio_source_url, work_dir, target_lufs=-16, fade_in_seconds=1.5, fade_out_seconds=3, gap_seconds=0, loop_count=None, mix_seconds=None):
    ...
    # acompressor (attack=10ms) — резкий транзиент (например, снейр на 2 и 4
    # долю — характерная черта лофая, которую убирать не нужно) не должен
    # звучать жёстким щелчком; alimiter — честный лимитер с lookahead и
    # быстрой атакой (5мс) для тех же коротких пиков.
